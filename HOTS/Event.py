...
def LoadNMNIST(NbTrainingData, NbTestingData, NbClusteringData, 
               OneOfEach=False, Path=None, OutOnePolarity=False, ListPolarities=None, verbose=0):
    '''
    ...
    def make_events(list_events, NbData, ListPolarities=ListPolarities, OutOnePolarity=OutOnePolarity):
        ...
        for idx in list_events:
            ...
            if OneOfEach:
                if events_digit.l not in selectid:
                    for idev in range(len(events_digit.t)):
                        event.time[idg] = events_digit.t[idev]*pow(10,-6)
                        ...
                        idg += 1
                    changeidx.append(len(events_digit.t))
                    label[idgl][0] = events_digit.l
                    idgl+=1
                    selectid.append(events_digit.l)
            else:
                for idev in range(len(events_digit.t)):
                    event.time[idg] = events_digit.t[idev]*pow(10,-6)
                    event.address[idg][0] = int(events_digit.y[idev])
    ...
    event_train, label_train = make_events(list_train, NbTrainingData, ListPolarities=ListPolarities, OutOnePolarity=OutOnePolarity)
    
    event_test, label_test = make_events(list_test, NbTestingData, ListPolarities=ListPolarities, OutOnePolarity=OutOnePolarity)

    event_cluster, _ = make_events(list_clust, NbClusteringData, ListPolarities=ListPolarities, OutOnePolarity=OutOnePolarity)
