...
def readATIS_td(file_name, orig_at_zero=True, drop_negative_dt=True, verbose=True):
    ...
    if header:
        ...
        if verbose:
            print('> Header exists. Event type is ' +
                  str(ev_type) + ', event size is ' + str(ev_size))
        if ev_size != 8:
...
def writeATIS_td(file_name, ts, coords, pol):
    ...
    for i in np.arange(0, ts.size):
        ...
        binevent = (int(ts[i]) & 0x00000000FFFFFFFF) + ((int(pol[i]) << 49) & 0x0002000000000000) + (
            (int(coords[i, 0]) << 32) & 0x000001FF00000000) + ((int(coords[i, 1]) << 41) & 0x0001FE0000000000)
...
def readATISdump(file_name, orig_at_zero=True, drop_negative_dt=True):
    ...
    for ev in np.arange(Nevents):
        ...
        if ev_type == EVT_TIME_HIGH:
            ...
            # MSB of the timestamp, shifted by 11 bits to make room for the LSB of the timestamp
            ts_MSB = (data & 0x0FFFFFFF) << 11
...
def plotHistogramAndMeansForDeltaTs(timestamps, max_dt=5000, time_window=1000, fignumbers=[1, 2, 3], my_dpi=96, figpixsize=[1400, 800], savename='noname', show_plots=False, dt_too_high=1000):
    ...
    # plot dts
    print("Figures...")
    plt.figure(fignumbers[0], figsize=(figpixsize[0] /
                                       my_dpi, figpixsize[1] / my_dpi), dpi=my_dpi)
    if max_obs_dt < 5000:
    ...
    dt_ticks = ["%d to %d µs" % (j, i)
                for i, j in zip(histobins[1:], histobins[:-1])]
    ...
    # plot events per ms
    plt.figure(fignumbers[1], figsize=(figpixsize[0] /
                                       my_dpi, figpixsize[1] / my_dpi), dpi=my_dpi)
    plt.plot(timestamps[index_segmented], events_per_ms,
             'bo-', fillstyle="none", label="no-overlap mean")
    plt.plot(timestamps[index_moving],
             events_per_ms_moving, 'r*-', label="moving mean")
    ...
    # plot timestamps
    plt.figure(fignumbers[2], figsize=(figpixsize[0] /
                                       my_dpi, figpixsize[1] / my_dpi), dpi=my_dpi)
    plt.plot(timestamps, np.arange(timestamps.size), 'r.', fillstyle="none")
    verticalbar = [0, timestamps.size]
    for gap in timestamps[index_gap_max_dt]:
        plt.plot([gap, gap], verticalbar, 'g-')
    plt.title('Timestamps. Vertical bars when no event occured for more than' +
              str(max_dt / 1000) + ' ms')
    plt.ylabel('index')
    plt.xlabel('time (µs)')
...
def displayStreamOfEvents(timestamps, coords, polarities, time_window=1000, fignumber=4):
    ...
    ani = animation.FuncAnimation(
        fig, func=update, frames=max_frame, repeat=False, interval=10)
    plt.show()
...
def denoise(ts, coords, min_dt=1000, radius=1, min_ev=1):
    ...
    for ii in np.arange(1, ts.size):
        ...
        #     print("no!")
        keepers[ii] = np.sum(
            ((ts[ii] - recent_events[x-radius:x+radius+1, y-radius:y+radius+1]) < min_dt)) > min_ev
    print("> To be removed:",  (ts.size - np.sum(keepers)), "events. (~",
          int(100.0 * float(ts.size - np.sum(keepers)) / float(ts.size)), "%)")
    return keepers
...
def displayFramesColorPol(frames, myvmin=-1, myvmax=1, frameinterval=10):
    ...
    # custom colormap
    # black, red, orange,...
    colors = [(0.5, 0.5, 0.5), (1, 0, 0), (0, 0.5, 0), (1, 0.5, 0), (1, 1, 0),
    ...
    im = ax.imshow(frames[:, :, 0], interpolation="none",
                   cmap=cm, vmin=myvmin, vmax=myvmax)
    #title = plt.title("")
    plt.xticks([], [])
    plt.yticks([], [])
    ...
    ani = animation.FuncAnimation(
        fig, func=update, frames=max_frame, repeat=True, interval=frameinterval)
