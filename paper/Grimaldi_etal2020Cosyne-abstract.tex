...
\begin{wrapfigure}{l}{.7\textwidth}
\includegraphics[width=.99\linewidth]{layerz.png}
\caption*
{
\label{fig:fig2}
}
...
Our SNN learns with the following rule when a feature or neuron $j$ is matched: $C_j \leftarrow C_j + \alpha(S_k-\beta_j C_j)$ where $ \alpha = \frac{0.01}{1+\frac{p_j}{2000}}$, $p_j$ being the number of times feature $j$ was activated. This weight change between neurons refers to Hebbian learning, our network is a simple feedforward SNN with Hebbian learning rule. Neurons will learn patterns from spiking input aka time-surfaces. When observing \ref{fig:fig2}(A), we can see that activation of the neurons is unbalanced. In some cases, learning can lead to grandmother cells and penalize the generalization of the network as well as learning when some neurons are hardly matched. As observed in \ref{fig:fig2}(D) adding an homeostatic gain favoring rarely activated neurons can improve learning of the weights and make a more efficient coding of the spiking input. 
%
